...
int main(){
    ...
    vector<Line2d> ans = commonTangents(c1, c2);
    vector<Point> res;
    for(Line2d x : ans){
        Line l({0,0}, {0,0});
        if(x.b != 0)    l = Line(Point(0, x.eval(0)), Point(1, x.eval(1)));
        else            l = Line(Point(-x.c/x.a, 0), Point(-x.c/x.a, 1));
        res.push_back(getCrossPoints(c1, l).first);
    }
    sort(res.begin(), res.end());
    res.erase(unique(res.begin(),res.end()),res.end());
    cout << fixed << setprecision(12);
    for(Point p : res)  cout << p.x << " " << p.y << endl;
    return 0;
}
