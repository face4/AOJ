...
bool ng(Point p){
    return equals(p.x, 100) || equals(p.x, -100) || equals(p.y, 100) || equals(p.y, -100);
}
...
int main(){
    ...
    while(cin >> n, n){
        ...
        for(int i = 0; i < n; i++){
            double ax, ay, bx, by;
            cin >> ax >> ay >> bx >> by;
            Segment cur(Point(ax,ay), Point(bx,by));
            vector<Point> vp;
            for(Segment s : v){
                if(intersect(s, cur))   vp.push_back(getCrossPoint(s, cur));
            }
            vector<Point> res;
            for(int j = 0; j < vp.size(); j++){
                ...
                for(int k = j+1; k < vp.size(); k++){
                    if(getDistance(vp[j], vp[k]) < EPS) valid = false;
                }
                if(valid)   ans++;
            }
            ans++;
