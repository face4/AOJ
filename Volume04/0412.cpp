...
int contains(Polygon g, Point p);   // O(n)
...
int main(){
    ...
    Polygon g;
    for(int i = 0; i < n; i++)  g.push_back(p[(piv+i)%n]);

    vector<double> ang;
    for(int i = 1; i < n; i++)  ang.push_back(arg(g[i]-g[0]));
    ...
    while(q--){
        double x, y;
        cin >> x >> y;
        x *= (1000000-100)/1000000.0;
        y *= (1000000-100)/1000000.0;
        int j = lower_bound(ang.begin(),ang.end(),arg(Point(x,y)-g[0]))-ang.begin();
        if(j+1 > n-1)   j = n-2;
        Polygon tri;
        tri.push_back(g[0]);
        tri.push_back(g[j]);
        tri.push_back(g[j+1]);
        // cout << tri[0].x << " " << tri[0].y << endl;
        // cout << tri[1].x << " " << tri[1].y << endl;
        // cout << tri[2].x << " " << tri[2].y << endl;
        cout << (contains(tri, Point(x,y))>0) << endl;
    }
    return 0;
}
